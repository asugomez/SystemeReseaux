...
#define CHECK_IFFAILED(status, msg)    \
...
/* ------------------------------------------------------------------------ */
const char *msgPi ="Que J'aime A Faire Apprendre Un NOMBRE Utile Aux SAGES";
...
int main(int argc, char *argv[])
{
    ...
    //init
    CHECK_IFFAILED(psemEcran = sem_open(SEM_FILENAME, O_CREAT | O_EXCL, S_IRWXU,1) , "sem_open()" );
    ...
    CHECK(sem_close(psemEcran),"sem_close");
    CHECK(sem_unlink(SEM_FILENAME), "sem_unlink()");
    return EXIT_SUCCESS;
}
...
void monThread (long  no)
{
    ...
    //lock
    CHECK(sem_wait(psemEcran),"sem_wait()");
    for (p = msgPi; *p != '\0'; p++) {
        //alphabetique? 
        if (isalpha(*p)) {
            n++;
    ...
    //unlock
    CHECK(sem_post(psemEcran),"sem_post()");
