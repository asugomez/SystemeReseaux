...
//1ere modification
    CHECK(sem_destroy(&buf.semBufferVide), "sem_destroy()");
    CHECK(sem_destroy(&buf.semBufferPlein), "sem_destroy()");
    return 0;
}
...
void  redacteur(long no)
{
    ...
    while  ( n != OVER ) {
        //mod
        CHECK(sem_wait(&buf.semBufferVide), "sem_wait()");
        if (n == MAX_VALUE)
            buf.buffer[i] = n = OVER;
        else
            buf.buffer[i] = n++;
        ...
        putLine(line);
        //mod 1
        CHECK(sem_post(&buf.semBufferPlein), "sem_post(Plein)");
...
void  lecteur(long no)
{
    ...
    while ( n != OVER ) {
        //tempo(10, 100);
        CHECK(sem_wait(&buf.semBufferPlein), "sem_wait()");
        n = buf.buffer[i];
        sprintf(line,"%*sLe lecteur n°%ld lit %d dans le buffer %d\n",
                5 * (int)(no + 1), "   ", no,   n, i);
        putLine(line);
        CHECK(sem_post(&buf.semBufferVide), "sem_post(Vide)");
...
void init(circBuffer_t * b)
{
    ...
    CHECK(sem_init(& (b->semBufferPlein),0,0), "sem_init(BufPlein)");
    CHECK(sem_init(& (b->semBufferVide),0,NBMAX_BUFFER), "sem_init(BufVide)");
